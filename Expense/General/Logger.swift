...
class Log {
    ...
    // tungxuan
    class func m(_ objects: Any..., filename: String = #file, line: Int = #line, column: Int = #column, funcName: String = #function) {
        if isLoggingEnabled {
            let arr = objects.map { "\($0)" }
            let s = arr.joined(separator: "\n")
            print("\(Date().toString()) \(LogEvent.s.rawValue)[\(sourceFileName(filePath: filename))]:\(line) \(column) \(funcName) -> \n\(s)\n")
        }
    }
    // end tungxuan
