...
public extension Date {
    ...
    */
    ...
    func ordinalDay() -> Int {
        ...
        let s1 = self.toString(format: .isoDate)
        let d1 = Date(fromString: s1, format: .isoDate)!
        let index = calender.ordinality(of: .day, in: .era, for: d1) ?? -1
        // add 1 because index start in 0
        return index + 1
    }
    
    static func fromOrdinalDay(_ i: Int) -> Date {
        return Calendar.current.date(from: .init(day: i))!
    }
    // end tungxuan
