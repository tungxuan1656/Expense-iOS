...
public class HUD: UIView {
	...
	let defaultLabelFontSize: CGFloat = 15
    ...
    public var contentColor = UIColor(white: 0.06, alpha: 0.7) {
		didSet {
	...
	public var delegate: HUDDelegate?
	...
	@discardableResult
	fileprivate class func show(addedToView view: UIView, animated: Bool) -> HUD {
		let hud = HUD(withView: view)
	...
	@discardableResult
	fileprivate class func hide(addedToView view: UIView, animated: Bool) -> Bool {
	...
	@discardableResult
	public class func hudForView(_ view: UIView) -> HUD? {
		let subviews = view.subviews.reversed()
		for subview in subviews {
			if (subview is HUD) {
				return subview as? HUD
			}
		}
	...
	// MARK: - tungxuan
	@discardableResult
    public class func show(_ view: UIView, title: String? = nil, animated: Bool = true, hide delay: TimeInterval = 10) -> HUD {
		let hud = HUD(withView: view)
		hud.removeFromSuperViewOnHide = true
		view.addSubview(hud)
		hud.show(animated: animated)
	...
	public class func showDone(_ view: UIView, hide delay: TimeInterval = 10, completion: (() -> Void)? = nil) {
		DispatchQueue.main.async {
            let hud = HUD.hudForView(view) ?? HUD.show(view)
            if #available(iOS 13.0, *) {
                let image = UIImage(systemName: "checkmark", withConfiguration: UIImage.SymbolConfiguration(pointSize: 50, weight: .bold))
                hud.mode = .customView
                let imageView = UIImageView(image: image)
                imageView.tintColor = UIColor(white: 0.06, alpha: 0.7)
                hud.customView = imageView
                hud.isSquare = true
                hud.label?.text = NSLocalizedString("Done", comment: "")
                if delay > 0 {
                    hud.hide(animated: true, afterDelay: delay)
                    Timer.scheduledTimer(withTimeInterval: delay + 0.5, repeats: false) { (_) in
                        if let c = completion {
                            c()
                        }
                    }
	...
	public class func showText(_ view: UIView, text: String, image: UIImage? = nil, hide delay: TimeInterval = 10, completion: (() -> Void)? = nil) {
		DispatchQueue.main.async {
            let hud = HUD.hudForView(view) ?? HUD.show(view)
            if let image = image {
                hud.mode = .customView
	...
	fileprivate func hide(animated: Bool, afterDelay delay: TimeInterval) {
	...
	fileprivate func showUsingAnimation(_ animation: Bool) {
    ...
    fileprivate func animateIn(_ animatingIn: Bool, withType: HudAnimation, completion: ((Bool) -> Void)?) {
    ...
    fileprivate func updateViews(forColor color: UIColor) {
...
public class ProgressHUDBackgroundView: UIView {
	...
	override init(frame: CGRect) {
		...
		style = .blur
		color = UIColor(white: 0.9, alpha: 0.9)
