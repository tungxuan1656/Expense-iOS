...
extension Dictionary where Key == String, Value == Any {
    func jsonString() -> String {
        do {
            let dataParams = try JSONSerialization.data(withJSONObject: self, options: [.fragmentsAllowed, .prettyPrinted])
    ...
    func jsonData() -> Data {
        do {
            return try JSONSerialization.data(withJSONObject: self, options: [.fragmentsAllowed, .prettyPrinted])
        } catch {
            return Data()
        }
    }
...
extension String {
    func decimalInputFormatting() -> String {
        ...
        let s = regex.stringByReplacingMatches(in: self, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSMakeRange(0, self.count), withTemplate: "")
        if let d = Double(s) {
            return d.converDecimal()
        }
        else {
            return self
        }
...
extension String {
    func toDateTimeString(current: String = "yyyy-MM-dd'T'HH:mm:ss.000Z", format: String = "HH:mm dd/MM/yyyy") -> String {
    ...
    func toDateTime(timeZone: TimeZone = TimeZone.current, format: String = "HH:mm dd/MM/yyyy") -> Date? {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = format
...
extension String {
    func unEmptyOrElse(_ v: String) -> String {
        return self != "" ? self : v
    }
}

extension Int {
    func positiveOrElse(_ v: Int) -> Int {
        return self > 0 ? self : v
    }
}

extension Double {
    func positiveOrElse(_ v: Double) -> Double {
        return self > 0 ? self : v
    }
}

extension Float {
    func positiveOrElse(_ v: Float) -> Float {
        return self > 0 ? self : v
    }
}

extension CGFloat {
    func positiveOrElse(_ v: CGFloat) -> CGFloat {
        return self > 0 ? self : v
    }
}
...
extension Bundle {
    ...
    var displayName: String {
        return object(forInfoDictionaryKey: "CFBundleDisplayName") as? String ?? "Web Player"
    }
    ...
    var appId: String {
        return bundleIdentifier ?? "com.mdcmedia.webplayer.ios"
    }
}
...
class Currency {
    ...
    func get(_ price: Double) -> String {
...
extension Currency {
    ...
    static let vnd = Currency("đ", minFractionDigits: 0, maxFractionDigits: 0, isRTL: true)
}
